function [curve, gof] = r_2_fit(E_data,i_data, a_H_plus, T, data_type)
...
if data_type == "Linear"
    ...
    fun = @(k_2_0_plus, alpha, k_1_0, x) n*F*gamma*k_2_0_plus*exp((1-alpha)*F*(x - E_rev)/(R*T))./(1 + (a_H_plus/a_H2O)*k_1_0*exp(-F*(x - E_rev)/(R*T)));
...
elseif data_type == "Logarithmic"
    ...
    fun = @(k_2_0_plus,alpha, k_1_0,x) log10(n*F*gamma*k_2_0_plus) + (1-alpha)*F*(x - E_rev)*log10(exp(1))/(R*T) - log10((1 + (a_H_plus/a_H2O)*k_1_0*exp(-F*(x - E_rev)/(R*T))));
    FT = fittype(fun, 'independent',{'x'}, 'coefficients',{'k_2_0_plus', 'alpha', 'k_1_0'});
