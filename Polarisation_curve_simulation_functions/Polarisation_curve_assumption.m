...
% based on the expression from Reksten but with the approximation whch
% leads to an extended version of the rds

% This was originally acidic, but because of the recent descovery of the
% alkaline model, I will turn this alkaline as well
% Will be based on no previous fitting
...
[Cherevko_curve_alkaline_reksten, Cherevko_gof_alkaline_reksten] = r_rekstek_assumption(Cherevko_E_alkaline, Cherevko_i_alkaline, Cherevko_OH_alkaline, Cherevko_T_alkaline, "Linear");
[Damjanovic_curve_alkaline_reksten, Damjanovic_gof_alkaline_reksten] = r_rekstek_assumption(Damjanovic_E_alkaline, Damjanovic_i_alkaline, Damjanovic_OH_alkaline, Damjanovic_T_alkaline, "Linear");
[Damjanovic_log_curve_alkaline_reksten, Damjanovic_log_gof_alkaline_reksten] = r_rekstek_assumption(Damjanovic_E_alkaline, Damjanovic_i_alkaline, Damjanovic_OH_alkaline, Damjanovic_T_alkaline, "Logarithmic");
[Schalenbach_curve_alkaline_reksten, Schalenbach_gof_alkaline_reksten] = r_rekstek_assumption(Schalenbach_E_alkaline,Schalenbach_i_alkaline, Schalenbach_a_OH_alkaline, Schalenbach_T_alkaline, "Linear");
...
hold on
fig_damjanovic_alkaline = plot(Damjanovic_curve_alkaline_reksten, "black");                        % Creating a fig to stor the plot of the curve fit (cfit element)
...
str_damjanovic_alkaline = ...                                                              % Creating a string element for the annotation
    sprintf("$R^{2}$ = %.5f", round(Damjanovic_gof_alkaline_reksten.rsquare, 5));
...
%--------------------------------------------------------------------------
% Damjanovic_log - Alkaline
figure("Name","Damjanovic Alkaline Fitting")                                               % Creates figure
scatter(Damjanovic_E_alkaline, log10(Damjanovic_i_alkaline), 45,...                               % Scatter plot of the sampled values from Damjanovic
    [0.9290 0.6940 0.1250], "^", "filled")
hold on
fig_damjanovic_alkaline = plot(Damjanovic_log_curve_alkaline_reksten, "black");                        % Creating a fig to stor the plot of the curve fit (cfit element)
set(fig_damjanovic_alkaline,'lineWidth',1);                                                % Changing the linewidth of the curve of the cfit
hold off
ax_damjanovic_alkaline = gca; % current axes                                               % Creating an ax with gca such that the fontsize can be changed
legend({'Data', 'Fitting'},...                                                             % Creating a legend for the graphs
    'Position', [.2 .65 .1 .1], 'Interpreter','latex', 'FontSize',15)
str_damjanovic_alkaline = ...                                                              % Creating a string element for the annotation
    sprintf("$R^{2}$ = %.5f", round(Damjanovic_log_gof_alkaline_reksten.rsquare, 5));
annotation('textbox', [.15 .8 .1 .1], 'String',str_damjanovic_alkaline,...                 % Creating an annotation, textbox, with the rsquare value from the cfit
    'Interpreter', 'latex','FitBoxToText','on', 'FontSize',15);
ax_damjanovic_alkaline.XAxis.FontSize = 12;                                                % Changing the tick size on the x-axis
ax_damjanovic_alkaline.YAxis.FontSize = 12;                                                % Changing the tick size on the y-axis
